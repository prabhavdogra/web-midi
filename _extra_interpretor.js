...
let down = false;
function exampleHandler({ type, device, a, b }) {
  if (device.manufacturer !== "Akai" || device.name !== "MPD218 Port A") return;
  if (!["note_on", "note_off", "aftertouch"].includes(type)) return;
  if (type === "note_on" || type === "note_off") {
    if (a.value === 44) {
      down = type === "note_on";
      console.log(b.type, b.ratio);
    }
  } else if (type === "aftertouch" && down) {
    console.log(a.type, a.ratio);
  }
}

let events = [];
const midi = new MIDI((event) => {
...
function render() {
  ...
  for (let i = events.length - 1; i >= 0; i--) {
    const { device, a, b, id, type } = events[i];
    el.innerHTML += `
      <tr>
        <td><small>
        ...
        <td><small>
          ${type} 
          ${a && a.value !== null ? ` | ${a.type}: ${a.value}` : ""}
          ${b && b.value !== null ? ` | ${b.type}: ${b.value}` : ""}
        </small></td>
      </tr>
    `;
  }
